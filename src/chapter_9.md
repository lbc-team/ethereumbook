...
智能合约代码是无情的。每个错误都可能导致金钱损失。您不应该像对待通用编程那样对待智能合约编程。用 Solidity 编写 [DApp](https://learnblockchain.cn/tags/DApp) 与用 [JavaScript](https://learnblockchain.cn/tags/JavaScript) 创建 Web 小部件不同。相反，您应该像在航空航天工程或任何类似无情的学科中那样，应用严格的工程和软件开发方法。一旦您“启动”了您的代码，您几乎无法修复任何问题。即使代码是可升级的，如果出现任何问题，您通常也没有多少时间做出响应。如果有人在您之前发现了您项目中的错误，漏洞利用可能会在单个交易或几个交易中展开，这意味着损害会在几秒钟内完成，远在您可以干预之前。
...
最近一个重入是唯一的攻击向量的漏洞利用是 2023 年 7 月发生的 Libertify 事件，这是一个被盗取了 40 万美元的 [DeFi](https://learnblockchain.cn/tags/DeFi?map=EVM) 协议。让我们检查一下被利用的函数的代码，看看它是如何发生的：
...
```solidity
1 abstract contract ERC4626 is [ERC20](https://learnblockchain.cn/tags/ERC20?map=EVM), IERC4626 {
...
当我们需要确定价格时，最好的选择是使用像 [Chainlink](https://learnblockchain.cn/tags/Chainlink)、RedStone、Pyth 等去中心化预言机。由于这些预言机是去中心化的，因此它们更难被攻击，因为攻击者需要控制网络中超过 50% 的节点。但它们也有局限性。例如，它们可能不适用于所有资产。在这种情况下，我们可以转向时间加权平均价格（TWAP）预言机。
...
通常，易受攻击的[预言机](https://learnblockchain.cn/tags/%E9%A2%84%E8%A8%80%E6%9C%BA)模块是协议本身的一部分，正如我们将在本例中看到的那样。当智能合约直接从 [Uniswap](https://learnblockchain.cn/tags/Uniswap?map=EVM) 等链上 AMM 协议获取资产价格时，就会发生常见的漏洞利用场景。想象一个 [Uniswap](https://learnblockchain.cn/tags/Uniswap?map=EVM) V2 池，其中有 4,000 USDC 和 1 ETH 的储备。智能合约可能会假设 1 ETH 价值 4,000 USDC。但是，如果推断出的价格用于进一步的状态改变操作，这种假设可能非常危险。在这种情况下，攻击者可以进行闪电贷执行大规模的兑换，改变池的余额，从而改变 ETH 的推断价格。易受攻击的协议依赖于这种被操纵的价格，然后会被攻击者利用。
...
对于更复杂的需求，[OpenZeppelin](https://learnblockchain.cn/tags/OpenZeppelin?map=EVM) 的 AccessControl 允许我们创建多个角色，每个角色具有不同的权限。基于角色的访问控制允许我们将特定任务分配给不同的用户，使其成为更大项目的理想选择。
...
一个全面的 24 小时[课程](https://learnblockchain.cn/courses)（270 多个[课程](https://learnblockchain.cn/courses)）
...
[OpenZeppelin](https://learnblockchain.cn/tags/OpenZeppelin?map=EVM) 的一个基于 [Solidity](https://learnblockchain.cn/tags/Solidity?map=EVM) 的战争游戏，玩家通过黑客智能合约级别来学习常见的漏洞

**Damn Vulnerable [DeFi](https://learnblockchain.cn/tags/DeFi?map=EVM)**

一个夺旗 (CTF) 平台，包含 18 个挑战，涵盖闪电贷、价格[预言机](https://learnblockchain.cn/tags/%E9%A2%84%E8%A8%80%E6%9C%BA)、治理、[NFT](https://learnblockchain.cn/tags/NFT) 等

**Capture the Ether**

一个经典的 CTF 风格的[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)安全游戏

**QuillCTF**

QuillAudits 的[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)安全谜题的集合
...
感谢它的更新，[Solidity](https://learnblockchain.cn/tags/Solidity?map=EVM) 编译器现在可以缓解诸如整数溢出和默认可见性问题之类的风险。这使我们能够删除本书第一版中提到的一些较旧的陷阱，并使用该空间来关注更多当前和相关的漏洞。无论如何，任何在智能合约领域工作的开发人员仍然需要知道和理解很多东西。通过遵循[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)设计和代码编写中的最佳实践，您将避免许多严重的陷阱和陷阱。

也许最基本的软件安全原则是最大限度地重用受信任的代码。在密码学中，这非常重要，以至于它已被浓缩成一句格言：“不要自己编写密码。”在[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)的情况下，这相当于从社区彻底审查过的免费库中获得尽可能多的收益。
