...
[Phantom](https://learnblockchain.cn/tags/Phantom?map=Solana) 是一个只能连接到以太坊以及其他非 EVM 链的钱包。
...
- 创建新账户时，首先仅向新地址发送一笔小额测试交易。收到测试交易后，尝试再次从该[账户](https://learnblockchain.cn/tags/账户?map=EVM)发送回去。[账户](https://learnblockchain.cn/tags/账户?map=EVM)创建可能会出错有很多原因，如果出错，最好以小额损失来发现。如果测试有效，一切都很好（也可以使用测试网以避免任何类型的损失）。
...
Sepolia 于 2021 年 10 月由以太坊核心开发人员推出，作为一个权威证明网络，此后已过渡到 [PoS](https://learnblockchain.cn/tags/PoS) 共识，镜像了以太坊的主网环境。
...
一旦你从 Sepolia 测试水龙头收到了你的第一个测试 ether，你就可以通过尝试将其发送回水龙头或任何其他地址来试验发送 ether。在此示例中，我们将尝试将一些测试网 ether 发送到 [Vitalik](https://learnblockchain.cn/tags/Vitalik?map=EVM) Buterin，如图 2-9 所示。
...
你在 MetaMask [钱包](https://learnblockchain.cn/tags/%E9%92%B1%E5%8C%85)中创建的帐户类型称为 *外部拥有帐户* (EOA)。EOA 是那些拥有私钥的帐户；拥有私钥意味着控制对资金或合约的访问。
...
合约具有地址，就像 EOA 一样。合约也可以发送和接收以太币，就像 EOA 一样。但是，当交易目的地是合约地址时，它会导致该合约在 [EVM](https://learnblockchain.cn/tags/EVM?map=EVM) 中 *运行*，使用交易——以及交易的数据——作为其输入。除了以太币之外，交易还可以包含 *数据*，指示要运行合约中的哪个特定函数以及要将哪些参数传递给该函数。通过这种方式，交易可以 *调用* 合约中的函数。

请注意，由于合约帐户没有私钥，因此它不能 *发起* 交易。只有 EOA 可以发起交易，但是合约可以通过调用其他合约来 *响应* 交易，从而构建复杂的执行路径。

在接下来的几节中，我们将编写我们的第一个合约。然后，你将学习如何使用你的 MetaMask [钱包](https://learnblockchain.cn/tags/%E9%92%B1%E5%8C%85)和 Sepolia 测试网络上的测试以太币来创建、资助和使用该合约。

## 一个简单的合约：一个测试以太币水龙头

[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)有几种不同的高级语言，所有这些语言都可以用于编写合约和生成 [EVM](https://learnblockchain.cn/tags/EVM?map=EVM) 字节码。你可以在第 7 章中阅读最突出和最有趣的语言。一种高级语言是[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)编程的绝对主导选择：Solidity。Solidity 由 Gavin Wood 创建，并且已成为[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)（及其他地区）中使用最广泛的语言。我们将使用 Solidity 编写我们的第一个合约。
...
这是一个非常简单的合约，它几乎和我们可以做到的最简单。它也是一个 *有缺陷的* 合约，展示了许多不良做法和安全漏洞。我们将在后面的章节中通过检查它的许多缺陷来学习。但是现在，让我们逐行查看此合约的作用以及其工作原理。你将很快注意到 [Solidity](https://learnblockchain.cn/tags/Solidity?map=EVM) 的许多元素与现有的编程语言（例如 [JavaScript](https://learnblockchain.cn/tags/JavaScript)、Java 或 C++）相似。
...
我们将在第 7 章中进行更多的 [Solidity](https://learnblockchain.cn/tags/Solidity?map=EVM) [智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)编程，并在第 9 章中了解最佳实践和安全注意事项。
