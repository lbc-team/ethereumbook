...
然而，近年来出现了一系列不同的虚拟机：[Solana](https://learnblockchain.cn/tags/Solana?map=Solana) VM、Wasm VM、[Cairo](https://learnblockchain.cn/tags/Cairo?map=Web3) VM和Move VM可能是最著名和最有趣的虚拟机，每个都有其优点和缺点。它们采用不同的智能合约开发方法：
...
这些替代虚拟机与EVM不同的另一个领域是交易的*并行化*。我们已经说过EVM按顺序处理交易，没有任何形式的并行化。一些项目看到了这个缺点并试图改进它。例如，[Solana](https://learnblockchain.cn/tags/Solana?map=Solana) VM和Move VM都可以处理交易的并行执行，即使这并不总是可能的——也就是说，当两个交易通过与相同的合约交互来修改同一块存储时，它们不能并行执行。
...
比特币开创了区块链技术中使用 Merkle 树的先河。 事实上，每个比特币区块都包含包含在同一区块中的所有交易的 Merkle 根，因此不能在不修改整个区块头（也损害 [PoW](https://learnblockchain.cn/tags/PoW)）的情况下修改任何一个。

### 以太坊中的 Merkle-Patricia trie

以太坊采用了相同的概念并将其应用于自身，并针对其特定需求进行了一些修改。 Merkle 树非常适合永不更改的永久数据，例如[比特币](https://learnblockchain.cn/tags/比特币?map=BTC)交易。 然而，以太坊状态不断变化
如你所见，这个合约所做的只是持有一个持久状态变量，该变量被设置为运行此合约的最后一个[账户](https://learnblockchain.cn/tags/账户?map=EVM)的地址。
...
首先，你需要确保你的计算机上安装了 [Rust](https://learnblockchain.cn/tags/Rust)。如果未安装，请运行以下命令：
...
Gas 是 Ethereum 的单位，用于衡量在 Ethereum blockchain 上执行操作所需的计算和存储资源。与仅考虑以千字节计的交易大小的[比特币](https://learnblockchain.cn/tags/比特币?map=BTC)相比，Ethereum 必须考虑交易和智能合约代码执行执行的每个计算步骤。
...
- *[Gas](https://learnblockchain.cn/tags/Gas?map=EVM) 价格* 是你将交易发送到 Ethereum 网络时愿意为每个 gas 单位支付的以太币金额。

> **提示**  
>
> 虽然 gas 有价格，但不能“拥有”或“花费”。[Gas](https://learnblockchain.cn/tags/Gas?map=EVM) 仅存在于 EVM 内部，作为正在执行的计算工作量的计数。发送者被收取以太币的交易费用，该费用被转换为 gas 以进行 EVM 核算，然后转换回以太币作为支付给验证者的交易费用。
...
> **注意**
>
> 如果没有代码内省，EOF 的未来版本可能会完全改变底层虚拟机，例如 [Cairo](https://learnblockchain.cn/tags/Cairo?map=Web3) VM 或 Wasm 或任何其他虚拟机，通过自动将所有合约的代码转换为具有相同功能的新虚拟机的代码。
...
这只是一个非常简单的例子，它显示了[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)开发人员在日常工作中必须克服的问题之一。尽管编译器在自动分析和管理这些情况方面变得越来越好，但 EVM 根据其当前的工作方式施加了一些硬性限制。
...
除了 EOF 之外，EVM 的未来是不确定的，并且将取决于开发人员和不同项目如何使用 EVM。但是，EVM 可以在一些有趣的领域进行扩展，例如 *zk-EVM*，它将提供附加到每个区块的[零知识证明](https://learnblockchain.cn/tags/%E9%9B%B6%E7%9F%A5%E8%AF%86%E8%AF%81%E6%98%8E)，以证明其正确执行。此外，[*EVMMAX*](https://github.com/jwasinger/EIPs/blob/evmmax-2/EIPS/eip-6601.md) 和 [*SIMD*](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-616.md) 将为 EVM 带来更多功能，使其能够更快地进行大量加密处理，这将特别有利于依赖加密的应用程序，例如隐私协议或 L2。

## 结论

在本章中，我们探索了 EVM，跟踪了各种[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)的执行，并了解了 EVM 是如何执行字节码的。我们还了解了 gas，[EVM](https://learnblockchain.cn/tags/EVM?map=EVM) 的会计机制，并了解了它如何解决停止问题并保护[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)免受 DoS 攻击。此外，我们分析了 EOF，了解了它如何尝试修复传统 [EVM](https://learnblockchain.cn/tags/EVM?map=EVM) 的不同流程。

接下来，在第 15 章中，我们将探索[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)用于实现去中心化共识的机制。
