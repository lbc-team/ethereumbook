...
def save_published_article(filename, lbc_article_id, link):
    """保存已发布文章记录"""
...
def publish_article(filename, force=False):
    ...
    if not force and is_article_published(filename):
        ...
        print(f"   链接: {published_info.get('link')}")
    ...
    handle = "ethbook"
    user_id = try_create_user(handle)
    link = "https://masteringethereum.xyz/" + filename.replace(".md", ".html")
    category_id = 5 # get_category_id("以太坊")
    ...
    if lbc_article_id:
        ...
        # 记录发布信息
        save_published_article(filename, lbc_article_id, link)
    else:
